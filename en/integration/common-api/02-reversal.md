...
<Warning> A **reversal** is not a **refund**. It can only be initiated if the **original transaction has not been completed successfully**. </Warning>

## Supported Scenarios

<Note> The `/trade/v1/reversal` API is currently only supported in the following scenarios. It applies to transactions in a non-completed state (e.g. scanned but not paid). </Note>
